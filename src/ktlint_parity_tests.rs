...
mod ktlint_parity {
    ...
    fn has_rule(v: &[Violation], rule: &str) -> bool {
        v.iter().any(|x| x.rule_id == rule)
    }
    fn no_rule(v: &[Violation], rule: &str) -> bool {
        !has_rule(v, rule)
    }
    ...
    #[test]
    fn op_disallowed() {
        assert!(has_rule(&lint("val foo1 = 1+2\n"), "standard:op-spacing"));
    }
    ...
    #[test]
    fn colon_allowed() {
        assert!(no_rule(&lint("class A : B\n"), "standard:colon-spacing"));
    }
    #[test]
    fn colon_disallowed() {
        assert!(has_rule(&lint("class A:B\n"), "standard:colon-spacing"));
    }

    // ── Double Colon Spacing ──
    #[test]
    fn double_colon_allowed() {
        assert!(no_rule(
            &lint("val foo = Foo::class\n"),
            "standard:spacing-around-double-colon"
        ));
    }
    #[test]
    fn double_colon_disallowed() {
        assert!(has_rule(
            &lint("val foo1 = Foo ::class\n"),
            "standard:spacing-around-double-colon"
        ));
    }
    ...
    // ── No Wildcard Imports ──
    #[test]
    fn wildcard_disallowed() {
        assert!(has_rule(
            &lint("import java.util.*\n\nclass Foo\n"),
    ...
    // ── No Semicolons ──
    #[test]
    fn semicolon_disallowed() {
        assert!(has_rule(&lint("val x = 1;\n"), "standard:no-semicolons"));
    }
    #[test]
    fn no_semicolon_allowed() {
        assert!(no_rule(&lint("val x = 1\n"), "standard:no-semicolons"));
    }

    // ── Trailing Spaces ──
    #[test]
    fn trailing_space_disallowed() {
        assert!(has_rule(
            &lint("val x = 1   \n"),
    ...
    // ── Final Newline ──
    #[test]
    fn final_newline_disallowed() {
        assert!(has_rule(&lint("class Foo"), "standard:final-newline"));
    }

    // ── Max Line Length ──
    #[test]
    fn max_line_disallowed() {
    ...
    #[test]
    fn consecutive_blank_disallowed() {
        assert!(has_rule(
            &lint("a\n\n\nb\n"),
    ...
    // ── Chain Wrapping ──
    #[test]
    fn chain_single_line_allowed() {
        assert!(no_rule(
            &lint("list.filter { it > 0 }.map { it * 2 }\n"),
    ...
    // ── String Template ──
    #[test]
    fn string_template_braces_allowed() {
        assert!(no_rule(
            &lint("val s = \"${name}\"\n"),
    ...
    // ── Dot Spacing ──
    #[test]
    fn dot_spacing_disallowed() {
        assert!(has_rule(
            &lint("val x = obj . method()\n"),
    ...
    #[test]
    fn consecutive_comments_disallowed() {
        assert!(has_rule(
            &lint("// comment 1\n// comment 2\nval x = 1\n"),
    ...
    #[test]
    fn single_line_block_comment_disallowed() {
        assert!(has_rule(
            &lint("/* single line block comment */\nval x = 1\n"),
    ...
    #[test]
    fn blank_before_decl_disallowed() {
        assert!(has_rule(
            &lint("val x = 1\nfun bar()\n"),
    ...
    // ── Property Naming ──
    #[test]
    fn property_camel_case_allowed() {
        assert!(no_rule(
            &lint("val myProperty = 1\n"),
            "standard:property-naming"
        ));
    }
    #[test]
    fn property_const_upper_allowed() {
        assert!(no_rule(
            &lint("const val MAX_COUNT = 100\n"),
            "standard:property-naming"
        ));
    }
    ...
    // ── Class Naming ──
    #[test]
    fn class_pascal_allowed() {
        assert!(no_rule(
            &lint("class MyViewModel\n"),
    ...
    // ── No Empty Class Body ──
    #[test]
    fn empty_class_body_disallowed() {
        assert!(has_rule(
            &lint("class Foo {}\n"),
