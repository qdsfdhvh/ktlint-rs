...
impl Rule for RangeOperatorSpacing {
    fn id(&self) -> &'static str { "standard:spacing-around-range-operator" }
            ...
            if i + 1 < bytes.len() && bytes[i] == b'.' && bytes[i + 1] == b'.' {
                ...
                if i > 0 && bytes[i - 1] == b' ' && i > 1 && bytes[i - 2] != b'.' {
                    violations.push(Violation {
                        file: String::new(), line: line_num, col,
                ...
                if i + 2 < bytes.len() && bytes[i + 2] == b' ' {
                    violations.push(Violation {
                        file: String::new(), line: line_num, col: col + 2,
                        rule_id: self.id().to_string(),
...
mod tests {
    ...
    fn check(s: &str) -> Vec<Violation> {
        let mut p = KotlinParser::new(); let t = p.parse(s); RangeOperatorSpacing.check(&t, s)
    }
    #[test] fn range_ok() { assert!(check("for (i in 1..10)\n").is_empty()); }
    #[test] fn space_before_range() {
