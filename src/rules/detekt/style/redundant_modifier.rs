...
impl Rule for RedundantModifier {
    fn id(&self) -> &'static str { "detekt:style:RedundantModifier" }
...
mod tests {
    ...
    #[test] fn public_class_ok() { assert!(c("public class Foo { public fun bar() {} }").is_empty()); }
    #[test] fn public_in_private_bad() { assert!(!c("private class Foo { public fun bar() {} }").is_empty()); }
    #[test] fn private_in_private_ok() { assert!(c("private class Foo { private fun bar() {} }").is_empty()); }
}
