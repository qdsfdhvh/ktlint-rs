        ...
        if let Some(child) = body.child(i) {
            ...
            if last == Section::Function && current != Section::Function {
                ...
                v.push(Violation {
                    ...
                    message: "Functions should be declared after properties and constructors"
                        .into(),
                    auto_fixable: false,
                });
            } else if last == Section::Init
                && (current == Section::Property || current == Section::Constructor)
            {
                let pos = child.start_position();
                v.push(Violation {
                    file: String::new(),
...
mod tests {
    ...
    fn c(s: &str) -> Vec<Violation> {
        ClassOrdering.check(&KotlinParser::new().parse(s), s)
    }
    #[test]
