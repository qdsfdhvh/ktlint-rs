...
impl Rule for InstanceOfCheckForException {
    ...
    fn check(&self, _tree: &Tree, source: &str) -> Vec<Violation> {
        ...
        for (i, line) in source.lines().enumerate() {
            let t = line.trim();
            if t.starts_with("catch (") || t.contains("catch (") {
                in_catch = true;
            }
        ...
        source
            ...
            .filter_map(|(i, line)| {
                ...
                if t == "TODO()"
                    ...
                    || t.contains("throw NotImplementedError")
        ...
        for (i, line) in source.lines().enumerate() {
            ...
            if in_catch && !catch_var.is_empty() && t.contains(&format!("throw {}", catch_var)) {
                v.push(Violation {
                    ...
                    message: format!(
                        "Caught exception '{}' re-thrown — use throw directly",
                        catch_var
                    ),
                    auto_fixable: false,
                });
...
impl Rule for ReturnFromFinally {
    ...
    fn check(&self, _tree: &Tree, source: &str) -> Vec<Violation> {
        ...
        for (i, line) in source.lines().enumerate() {
            let t = line.trim();
            if t.starts_with("} finally {") || t == "finally {" {
                in_finally = true;
            }
...
impl Rule for TooGenericExceptionThrown {
    ...
    fn check(&self, _tree: &Tree, source: &str) -> Vec<Violation> {
        source
            ...
            .filter_map(|(i, line)| {
                let t = line.trim();
                if (t.contains("throw Exception(")
                    || t.contains("throw RuntimeException(")
                    || t.contains("throw Throwable("))
                    && !t.is_empty()
                {
                    Some(Violation {
                        file: String::new(),
