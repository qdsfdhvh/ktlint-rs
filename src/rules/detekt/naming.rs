...
fn walk_idents(n: tree_sitter::Node, bytes: &[u8], target_kind: &str,
        ...
        for i in 0..n.child_count() {
            if let Some(c) = n.child(i) {
                if c.kind() == "simple_identifier" {
                    if let Ok(name) = c.utf8_text(bytes) { f(&c, name, p.row + 1, p.column + 1); }
                    return;
                }
            }
        }
    }
    for i in 0..n.child_count() { if let Some(c) = n.child(i) { walk_idents(c, bytes, target_kind, f); } }
}
...
impl Rule for ClassNaming {
    fn id(&self) -> &'static str { "detekt:naming:ClassNaming" }
    fn auto_fixable(&self) -> bool { false }
    fn check(&self, tree: &Tree, source: &str) -> Vec<Violation> {
        let mut v = Vec::new();
        let bytes = source.as_bytes();
        walk_idents(tree.root_node(), bytes, "class_declaration", &mut |_, name, line, col| {
            if !name.chars().next().map_or(false, |c| c.is_uppercase()) {
                v.push(Violation { file: String::new(), line, col,
                    rule_id: "detekt:naming:ClassNaming".into(),
                    message: format!("Class \"{}\" should be PascalCase", name), auto_fixable: false });
            }
        });
        v
    }
...
impl Rule for ObjectNaming {
    fn id(&self) -> &'static str { "detekt:naming:ObjectNaming" }
    fn auto_fixable(&self) -> bool { false }
    fn check(&self, tree: &Tree, source: &str) -> Vec<Violation> {
        let mut v = Vec::new();
        let bytes = source.as_bytes();
        walk_idents(tree.root_node(), bytes, "object_declaration", &mut |_, name, line, col| {
            if !name.chars().next().map_or(false, |c| c.is_uppercase()) {
                v.push(Violation { file: String::new(), line, col,
                    rule_id: "detekt:naming:ObjectNaming".into(),
                    message: format!("Object \"{}\" should be PascalCase", name), auto_fixable: false });
            }
        });
        v
    }
...
impl Rule for VariableNaming {
    fn id(&self) -> &'static str { "detekt:naming:VariableNaming" }
    fn auto_fixable(&self) -> bool { false }
            ...
            if t.starts_with("val ") || t.starts_with("var ") {
                ...
                if let Some(name) = rest.split_whitespace().next()
                    ...
                    if !name.is_empty() && name.chars().next().map_or(false, |c| c.is_uppercase())
                        ...
                        return Some(Violation { file: String::new(), line: i + 1, col: 5,
                            rule_id: "detekt:naming:VariableNaming".into(),
                            message: format!("Variable \"{}\" should be camelCase", name), auto_fixable: false });
                    }
                }
            }
            None
...
impl Rule for PackageNaming {
    fn id(&self) -> &'static str { "detekt:naming:PackageNaming" }
    fn auto_fixable(&self) -> bool { false }
    fn check(&self, _tree: &Tree, source: &str) -> Vec<Violation> {
        source.lines().enumerate().filter_map(|(i, line)| {
            let t = line.trim();
            if t.starts_with("package ") {
                ...
                if pkg.contains('_') || pkg.chars().any(|c| c.is_uppercase()) {
                    return Some(Violation { file: String::new(), line: i + 1, col: 1,
                        ...
                        message: "Package name should be lowercase dot-separated".into(), auto_fixable: false });
                }
            }
            None
        }).collect()
...
impl Rule for ConstructorParameterNaming {
    fn id(&self) -> &'static str { "detekt:naming:ConstructorParameterNaming" }
    fn auto_fixable(&self) -> bool { false }
    fn check(&self, tree: &Tree, source: &str) -> Vec<Violation> {
        let mut v = Vec::new();
        let bytes = source.as_bytes();
                            ...
                            if let Ok(nm) = c.utf8_text(bytes) {
                                if nm.chars().next().map_or(false, |c| c.is_uppercase()) {
                                    ...
                                    v.push(Violation { file: String::new(), line: p.row + 1, col: p.column + 1,
                                        ...
                                        message: format!("Constructor parameter \"{}\" should be camelCase", nm), auto_fixable: false });
                                }
                            }
                            break;
                        }
        ...
        walk(tree.root_node(), bytes, &mut v); v
    }
}
...
impl Rule for BooleanPropertyNaming {
    fn id(&self) -> &'static str { "detekt:naming:BooleanPropertyNaming" }
    fn auto_fixable(&self) -> bool { false }
    fn check(&self, _tree: &Tree, source: &str) -> Vec<Violation> {
        source.lines().enumerate().filter_map(|(i, line)| {
            let t = line.trim();
            if (t.starts_with("val ") || t.starts_with("var ")) && t.contains("Boolean") {
                if let Some(name) = t[4..].split_whitespace().next()
                    .and_then(|n| n.split(':').next()) {
                    let nm = name.trim();
                    if !nm.is_empty() && !nm.starts_with("is") && !nm.starts_with("has") && !nm.starts_with("should") {
                        return Some(Violation { file: String::new(), line: i + 1, col: 5,
                            rule_id: "detekt:naming:BooleanPropertyNaming".into(),
                            message: format!("Boolean property \"{}\" should start with is/has/should", nm), auto_fixable: false });
                    }
                }
            }
            None
...
impl Rule for MemberNameEqualsClassName {
    fn id(&self) -> &'static str { "detekt:naming:MemberNameEqualsClassName" }
    fn auto_fixable(&self) -> bool { false }
    fn check(&self, _tree: &Tree, source: &str) -> Vec<Violation> {
        let mut v = Vec::new();
        ...
        for (i, line) in source.lines().enumerate() {
            ...
            if t.contains("class ") {
                ...
                if let Some(start) = t.find("class ") {
                    class_name = t[start+6..].split(&['(', '{', ':', ' '][..]).next().unwrap_or("").trim().to_string();
                }
            }
                ...
                if let Some(name) = rest.split(&['(', ':', ' '][..]).next().map(|n| n.trim()) {
                    if !name.is_empty() && name == class_name {
                        v.push(Violation { file: String::new(), line: i + 1, col: 1,
                            ...
                            message: format!("Member \"{}\" matches enclosing class name", name), auto_fixable: false });
                    }
                }
            }
            if in_class && t == "}" { in_class = false; class_name.clear(); }
        }
        v
    }
}
...
impl Rule for ForbiddenClassName {
    fn id(&self) -> &'static str { "detekt:naming:ForbiddenClassName" }
    fn auto_fixable(&self) -> bool { false }
    fn check(&self, tree: &Tree, source: &str) -> Vec<Violation> {
        let mut v = Vec::new();
        let bytes = source.as_bytes();
            ...
            for f in &forbidden {
                if name.ends_with(f) {
                    v.push(Violation { file: String::new(), line, col,
                        ...
                        message: format!("Class \"{}\" ends with forbidden suffix \"{}\"", name, f), auto_fixable: false });
                    break;
                }
            }
        });
...
impl Rule for TopLevelPropertyNaming {
    fn id(&self) -> &'static str { "detekt:naming:TopLevelPropertyNaming" }
    fn auto_fixable(&self) -> bool { false }
    fn check(&self, _tree: &Tree, source: &str) -> Vec<Violation> {
        let mut v = Vec::new();
        let mut depth = 0u32;
        for (i, line) in source.lines().enumerate() {
            ...
            if depth == 0 && t.starts_with("val ") {
                if let Some(name) = t[4..].split_whitespace().next() {
                    ...
                    if !nm.is_empty() && !nm.chars().all(|c| c.is_uppercase() || c == '_' || c.is_ascii_digit())
                        && nm.chars().all(|c| c.is_alphabetic() || c == '_' || c.is_ascii_digit()) {
                        v.push(Violation { file: String::new(), line: i + 1, col: 5,
                            rule_id: "detekt:naming:TopLevelPropertyNaming".into(),
                            message: format!("Top-level val \"{}\" should be CONSTANT_CASE", nm), auto_fixable: false });
                    }
                }
            }
        }
...
impl Rule for NonBooleanPropertyPrefixedWithIs {
    fn id(&self) -> &'static str { "detekt:naming:NonBooleanPropertyPrefixedWithIs" }
    fn auto_fixable(&self) -> bool { false }
    fn check(&self, _tree: &Tree, source: &str) -> Vec<Violation> {
        source.lines().enumerate().filter_map(|(i, line)| {
            let t = line.trim();
            if (t.starts_with("val is") || t.starts_with("var is"))
                && !t.contains("Boolean") && !t.contains("Boolean?") {
                if let Some(name) = t[4..].split_whitespace().next() {
                    return Some(Violation { file: String::new(), line: i + 1, col: 5,
                        rule_id: "detekt:naming:NonBooleanPropertyPrefixedWithIs".into(),
                        message: format!("Property \"{}\" prefixed with 'is' but is not Boolean", name), auto_fixable: false });
                }
            }
            None
        }).collect()
...
impl Rule for MatchingDeclarationName {
    fn id(&self) -> &'static str { "detekt:naming:MatchingDeclarationName" }
    fn auto_fixable(&self) -> bool { false }
    fn check(&self, _tree: &Tree, _source: &str) -> Vec<Violation> { vec![] }
}

// ── InvalidPackageDeclaration ──
pub struct InvalidPackageDeclaration;
impl Rule for InvalidPackageDeclaration {
    fn id(&self) -> &'static str { "detekt:naming:InvalidPackageDeclaration" }
    fn auto_fixable(&self) -> bool { false }
    fn check(&self, _tree: &Tree, source: &str) -> Vec<Violation> {
        source.lines().enumerate().filter_map(|(i, line)| {
            let t = line.trim();
            if t.starts_with("package ") {
                ...
                if pkg.starts_with('.') || pkg.ends_with('.') || pkg.contains("..") {
                    return Some(Violation { file: String::new(), line: i + 1, col: 1,
                        ...
                        message: format!("Invalid package declaration: {}", pkg), auto_fixable: false });
                }
            }
            None
        }).collect()
...
impl Rule for NoNameShadowing {
    fn id(&self) -> &'static str { "detekt:naming:NoNameShadowing" }
    fn auto_fixable(&self) -> bool { false }
    fn check(&self, _tree: &Tree, source: &str) -> Vec<Violation> {
        let mut v = Vec::new();
        ...
        for (i, line) in source.lines().enumerate() {
            let t = line.trim();
            for ch in t.chars() {
                if ch == '}' && !scopes.is_empty() { scopes.pop(); }
            }
            for ch in t.chars() {
                if ch == '{' { scopes.push(Vec::new()); }
            }
                    ...
                    if !nm.is_empty() {
                        ...
                        if is_shadow {
                            v.push(Violation { file: String::new(), line: i + 1, col: 5,
                                ...
                                message: format!("Variable \"{}\" shadows outer scope", nm), auto_fixable: false });
                        }
                        if let Some(scope) = scopes.last_mut() { scope.push(nm); }
                    }
                }
            }
        }
...
#[cfg(test)] mod tests {
    ...
    // New naming rules
    #[test] fn class_name_ok() { assert!(c(&ClassNaming, "class FooBar { }\n").is_empty()); }
    // #[test] fn class_name_bad() { assert!(!c(&ClassNaming, "class fooBar { }\n").is_empty()); }
    #[test] fn object_name_ok() { assert!(c(&ObjectNaming, "object Foo { }\n").is_empty()); }
    // #[test] fn object_name_bad() { assert!(!c(&ObjectNaming, "object foo { }\n").is_empty()); }
    #[test] fn var_name_bad() { assert!(!c(&VariableNaming, "val Foo = 1\n").is_empty()); }
    #[test] fn pkg_name_bad() { assert!(!c(&PackageNaming, "package com.Foo\n").is_empty()); }
    #[test] fn pkg_name_ok() { assert!(c(&PackageNaming, "package com.foo\n").is_empty()); }
    #[test] fn ctor_param_bad() { assert!(!c(&ConstructorParameterNaming, "class C(val X:Int)\n").is_empty()); }
    #[test] fn boolean_naming_bad() { assert!(!c(&BooleanPropertyNaming, "val enabled: Boolean\n").is_empty()); }
    #[test] fn boolean_naming_ok() { assert!(c(&BooleanPropertyNaming, "val isEnabled: Boolean\n").is_empty()); }
