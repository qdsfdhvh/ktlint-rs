...
impl Rule for IJTrailingComma {
    fn id(&self) -> &'static str { "standard:ij_kotlin_allow_trailing_comma" }
    fn check(&self, _t: &tree_sitter::Tree, s: &str) -> Vec<Violation> {
        let mut v=Vec::new();
        for(i,ln) in s.lines().enumerate() {
            let t=ln.trim();
            if (t.starts_with("fun ")||t.starts_with("class ")) && t.contains(',') && t.contains(')') {
...
#[cfg(test)] mod tests { use super::*; use crate::parser::KotlinParser;
    fn c(s:&str)->Vec<Violation>{let mut p=KotlinParser::new();IJTrailingComma.check(&p.parse(s),s)}
    #[test] fn ok() { assert!(c("fun f(a: Int, b: String)\n").is_empty()); }
    #[test] fn trailing() { assert!(!c("fun f(a: Int, b: String,)\n").is_empty()); }
}
