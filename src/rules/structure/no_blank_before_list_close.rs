...
impl Rule for NoBlankBeforeListClose {
    fn id(&self) -> &'static str { "standard:no-blank-line-before-list-closing" }
...
impl NoBlankBeforeListClose {
    fn walk(node: tree_sitter::Node, source: &str, v: &mut Vec<Violation>) {
        ...
        if k == "value_arguments" || k == "function_value_parameters" {
            ...
            if c > 0 { if let Some(last) = node.child(c-1) {
                if last.kind() == ")" {
                            ...
                            if prev.kind() != "(" && prev.kind() != "comment" {
                                if pr < lr - 1 {
                                    v.push(Violation { file: String::new(), line: lr+1, col: 1,
                                        ...
                                        auto_fixable: true,
                                        message: "Unexpected blank line before closing \")\"".into(),
                                    });
                                }
                                break;
                            }
        ...
        for i in 0..node.child_count() { if let Some(c) = node.child(i) { Self::walk(c, source, v); } }
    }
}
#[cfg(test)] mod tests { use super::*; use crate::parser::KotlinParser;
    fn c(s: &str) -> Vec<Violation> { let mut p=KotlinParser::new(); NoBlankBeforeListClose.check(&p.parse(s), s) }
    #[test] fn ok() { assert!(c("fun f(a: Int,\n      b: String\n)\n").is_empty()); }
    #[test] fn bad() { assert!(!c("fun f(a: Int,\n\n)\n").is_empty()); }
}
