    ...
    fn check(&self, _tree: &tree_sitter::Tree, source: &str) -> Vec<Violation> {
        ...
        for (i, line) in source.lines().enumerate() {
            ...
            if (trimmed.ends_with("{}") || trimmed.ends_with("{ }"))
                && (trimmed.contains("class ") || trimmed.contains("interface ") || trimmed.contains("object "))
            {
...
mod tests {
    ...
    fn check(s: &str) -> Vec<Violation> {
        let mut p = KotlinParser::new(); let t = p.parse(s); NoEmptyClassBody.check(&t, s)
    }
    #[test] fn non_empty_class() { assert!(check("class Foo { val x = 1 }\n").is_empty()); }
    #[test] fn empty_class() {
