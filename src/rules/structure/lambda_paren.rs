...
impl Rule for LambdaParen {
    fn id(&self) -> &'static str { "standard:no-unnecessary-parentheses-before-trailing-lambda" }
                ...
                if let Some(pos) = t.find(") {") {
                    ...
                    if i + 1 < source.lines().count() {
                        violations.push(Violation {
                            file: String::new(), line: i + 1, col: pos + 2,
...
mod tests {
    ...
    fn check(s: &str) -> Vec<Violation> { let mut p=KotlinParser::new(); LambdaParen.check(&p.parse(s), s) }
    #[test] fn no_paren_lambda() { assert!(check("list.forEach { it }\n").is_empty()); }
}
