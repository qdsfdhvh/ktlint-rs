...
mod tests {
    ...
    fn check(s: &str) -> Vec<Violation> {
        let mut p = KotlinParser::new(); let t = p.parse(s); NoEmptyFirstLineInClassBody.check(&t, s)
    }
    #[test] fn no_blank_first_line() { assert!(check("class Foo {\n    val x = 1\n}\n").is_empty()); }
    #[test] fn blank_first_line() {
