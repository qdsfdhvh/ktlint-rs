...
impl Rule for NoLeadingEmptyLinesInMethod {
    fn id(&self) -> &'static str { "standard:no-leading-empty-lines-in-method" }
    ...
    fn check(&self, _tree: &tree_sitter::Tree, source: &str) -> Vec<Violation> {
        ...
        for (i, line) in lines.iter().enumerate() {
            let t = line.trim();
            if (t.starts_with("fun ") || t.starts_with("init {")) && t.ends_with('{')
...
mod tests {
    ...
    fn check(s: &str) -> Vec<Violation> { let mut p=KotlinParser::new(); NoLeadingEmptyLinesInMethod.check(&p.parse(s), s) }
    #[test] fn no_blank() { assert!(check("fun foo() {\n    doA()\n}\n").is_empty()); }
    #[test] fn has_blank() { let v=check("fun foo() {\n\n    doA()\n}\n"); assert!(!v.is_empty()); }
}
