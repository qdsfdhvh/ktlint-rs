...
def generate(binary: pathlib.Path) -> tuple[dict[str, Any], str]:
    ...
    for rule_id in oracle_ids:
        ...
        if disabled:
            status = "disabled-by-oracle"
        elif not rust_ids:
            status = "missing"
        else:
            # Registration is evidence of code presence only, never parity proof.
            status = "partial"
        # The rule never fires in the Oracle build — effectively disabled-by-oracle.
...
def render_markdown(manifest: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- Disabled by oracle config: **{statuses.get('disabled-by-oracle', 0)}**",
    ...
    for rule in ordered:
        ...
        lines.append(
            "| {priority} | `{rule_id}` | {enabled} | {status} | {owners} | {formatter} | {coverage}/6 | {fixtures} | {hits} |".format(
                ...
                rule_id=rule["id"],
                enabled="yes" if rule["oracleEnabled"] else "no",
                status=rule["status"],
                owners=owners,
                formatter=formatter,
